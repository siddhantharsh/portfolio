...
export const Timeline = ({ data, isCertification }) => {
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky z-40 flex flex-col items-center self-start max-w-xs md:flex-row top-40 lg:max-w-sm md:w-full">
              ...
              {isCertification ? (
                <div className="flex-col hidden md:flex md:pl-20 md:text-4xl text-white">
                  <h3 className="text-3xl text-neutral-200 whitespace-nowrap overflow-hidden text-ellipsis bg-black/40 backdrop-blur-sm rounded-lg px-4 py-2">
                    {item.title}
                  </h3>
                </div>
              ) : (
            ...
            {!isCertification && (
              <div className="relative w-full pl-20 pr-4 md:pl-4">
                ...
                {item.contents.map((content, index) => (
                  <p className="mb-3 font-normal text-neutral-200 text-lg md:text-xl lg:text-2xl" key={index}>
